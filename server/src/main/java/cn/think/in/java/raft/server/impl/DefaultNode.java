...
public class DefaultNode implements Node, ClusterMembershipChanges {
    ...
    /**
     * 日志复制——附加日志 leader实现
     * 当用户向leader发来一条KV数据，leader把KV数据包装成一条附加日志
     * （有一个 ClientKVAck handlerClientRequest(ClientKVReq request) 接口，用于接收用户的 KV 数据）
     * 并行地将数据发送给它的所有跟随者
     * 如果超过半数返回成功，就提交这条数据
     *
     */
...
//                        replicationFailQueue.offer(model);
    ...
    /**
     * 日志复制实现——心跳
     * 请求者向所有节点发送的 内容为空的日志 就是心跳 定时任务 用来确定自己的leader地位
     *
     */
